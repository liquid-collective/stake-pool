...
setup_test_validator() {
  solana-test-validator -c FiSAMuamQakzK54tLc1yBVBiWEUoANDnGptbBdR83DES -c EmiU8AQkB2sswTxVB6aCmsAJftoowZGGDXuytm6X65R3 --url devnet --slots-per-epoch 32 --quiet --reset &
  # Uncomment to use a locally built stake program
  #solana-test-validator --bpf-program FiSAMuamQakzK54tLc1yBVBiWEUoANDnGptbBdR83DES ../../../target/deploy/spl_stake_pool.so --slots-per-epoch 32 --quiet --reset &
  pid=$!
